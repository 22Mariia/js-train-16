...
function filterStudentsByGrade(students, grade) {
  ...
  const filterStudent = students.filter((students) =>
...
function logArrayElements(arr) {
  ...
  if(!Array.isArray(arr)){
    console.erroe("Аргумент має бути масивом!");
    return null;
  }
...
function warnAboutMaxArrayLength(arr, maxLength) {
 if(!Array.isArray(arr)&& !Number.isInteger(maxLength)){
...
function displayGroupedInfo(students) {
  for(let  velue of students){
    console.group(`Студент: ${students.name}`);
    console.log(`Вік: ${students.age}`);
    console.log(`Предмет: ${students.subject}`);
    console.groupEnd();
  }
...
//  for (const value of products){value =>
...
function countVowelsAndConsonants(word) {
  ...
  if(str.includes(word.toLowerCase()) === true){
    console.count(str);
  }else{
